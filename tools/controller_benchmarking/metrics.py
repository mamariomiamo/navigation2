...
def getControllerResults(navigator, path, controllers, start_pose, dynamic_obstacles=False):
    ...
    while (
        abs(start_pose.pose.position.x - after_reset_pose.position.x) > 0.01
        and abs(start_pose.pose.position.y - after_reset_pose.position.y) > 0.01
    ):
...
def main():
    ...
    map_path = (
        os.getcwd() + "/" + glob.glob("**/maps/" + map_name.value + ".yaml", recursive=True)[0]
    )
    ...
    while len(planner_results) != random_pairs:
        ...
        print("costmap.shape[0]: ", costmap.shape[0], "costmap.shape[1]: ", costmap.shape[1])
        ...
        (
            ...
            task_poses,
            task_local_costmaps,
        ) = getControllerResults(navigator, result[0], controllers, start, dynamic_obstacles)
