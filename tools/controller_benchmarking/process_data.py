...
def reshapeList(input_list: list, n_controllers: int) -> np.array:
    input_list = np.asarray(input_list)
    # input_list.resize(int(input_list.size / n_controllers), n_controllers)

    input_list.resize(4, 2)
    return input_list
...
def main():
    ...
    speeds_x = reshapeList(speeds_x, len(controllers))
    ...
    controller_paths_length = []
    for controller_path, success in zip(controller_paths, flat_tasks_results):
        # print(success)
        # print("controller_path " ,controller_path)
        if success:
            controller_paths_length.append(getPathLength(controller_path))
        else:
            controller_paths_length.append(np.nan)
    # controller_paths_length = refactorArray(controller_paths_length, total_paths, len(controllers))
    controller_paths_length = reshapeList(controller_paths_length, len(controllers))
    # Distance from obstacles

    # TODO take into account failed navigations (no error is produced)
    # Minimum distance
    (
        controller_obstacles_distances,
        controller_obstacles_distances_avg,
        controller_obstacles_distances_std,
    ) = getObstacleDistances(tasks_controller_local_costmaps, local_costmap_resolution)
    ...
    controller_ME_linear_jerk = reshapeList(controller_ME_linear_jerk, len(controllers))
    controller_ME_angular_jerk = reshapeList(controller_ME_angular_jerk, len(controllers))
    # Single axis array
    flat_poses = []
    for task_poses in tasks_poses:
        for controller_poses in task_poses:
    ...
    for i in range(0, len(controllers)):
        controller_benchmark_table.append(
            [
                controllers[i],
                np.sum(tasks_results, axis=0)[i] / len(tasks_results),
                np.nanmean(speeds_x, axis=0)[i],
                np.nanmean(controller_paths_length, axis=0)[i],
                np.nanmean(task_times, axis=0)[i],
                ...
                np.nanmean(controller_ME_linear_jerk, axis=0)[i],
                np.nanmean(controller_ME_angular_jerk, axis=0)[i],
            ]
        )
    # Visualize results
    ...
    print(
        tabulate(controller_benchmark_table, headers="firstrow", showindex="always", floatfmt=".3f")
    )
